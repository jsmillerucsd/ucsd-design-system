...
The through-line: **`@ucsd/tokens` ships plain CSS custom properties.** Nothing about them is framework-specific. For the rules and the full token list, read [`DESIGN.md`](../../DESIGN.md).
...
Start from a pattern in [`../layouts/`](../layouts/README.md) — it gives you the region map, the required landmarks, and the content-model fields. Then render it with whichever of the above fits your platform; most CMS templates end up on the Bootstrap CDN path, since there's usually no Node build step available.
